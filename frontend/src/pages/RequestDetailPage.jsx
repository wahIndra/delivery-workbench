...
function ModuleCard({ title, path, desc }) {
  return (
    <Link to={path} className="card animate-slide-up group" style={{ display: 'flex', flexDirection: 'column', textDecoration: 'none', color: 'inherit', height: '100%' }}>
      <div className="flex items-center gap-3 mb-2">
        ...
        <h4 className="m-0 text-gray-800 group-hover:text-blue-700 transition-colors">{title}</h4>
      </div>
      <p style={{ fontSize: '0.875rem', color: 'var(--text-secondary)', margin: 0, flex: 1 }}>{desc}</p>
      <div className="mt-4 text-xs font-semibold text-blue-600 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity translate-y-2 group-hover:translate-y-0 duration-300">
